"""v4 governance: tool_credentials table

Revision ID: 0018_tool_credentials
...
(Step 10). The vault encrypts ``secret_ciphertext`` with the existing
``SECRETS_MASTER_KEY`` Fernet key — the same key already used by other
secret stores in this codebase.
...
revision = "0018_tool_credentials"
...
def upgrade() -> None:
    op.create_table(
        ...
        sa.UniqueConstraint("user_id", "name", name="uq_tool_credentials_user_name"),
    )
    op.create_index("ix_tool_credentials_user", "tool_credentials", ["user_id"])


def downgrade() -> None:
    op.drop_index("ix_tool_credentials_user", table_name="tool_credentials")
